...
def _init_redis_sentinel(redis_kwargs) -> redis.Redis:
    ...
    # NOTE: redis-py Sentinel uses `sentinel_kwargs` for authenticating to Sentinel.
    # The `password` parameter is used for authenticating to the discovered Redis master.
    redis_password = redis_kwargs.get("password")
    sentinel_kwargs = (
        {"password": sentinel_password} if sentinel_password is not None else None
    )
